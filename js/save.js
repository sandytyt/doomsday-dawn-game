/* ============================================
   末日黎明：喪屍浩劫 — 存檔與同步模組 (save.js)
   職責：處理 JSON 匯出/匯入、本機命名存檔、Notion 資料庫讀寫與備份
   ============================================ */
  ...
  reader.onload = function (event) {
    try {
      ...
      if (gameState.isTestMode || key) {
        ...
        if (typeof rebuildNarrativeFromHistory === 'function') rebuildNarrativeFromHistory();
        if (typeof renderOptions === 'function') renderOptions(gameState.lastOptions, makeOptionClickHandler);
...
function getNamedSaves() {
  try { return JSON.parse(localStorage.getItem(NAMED_SAVES_KEY) || '{}'); } 
  catch (e) { return {}; }
}
...
function switchSaveTab(tab) {
  currentSaveTab = tab;
  updateSaveTabUI();
  if (tab === 'local') renderLocalSaveList();
  else renderNotionSaveList();
}
...
function renderLocalSaveList() {
  ...
  if (names.length === 0) {
    ...
    emptyEl.className = 'named-save-empty'; emptyEl.textContent = '目前沒有任何命名存檔';
    dom.namedSaveList.appendChild(emptyEl); return;
  }
  names.forEach(function (name) {
    ...
    loadBtn.className = 'btn-secondary named-save-load-btn'; loadBtn.type = 'button'; loadBtn.textContent = '讀取';
    loadBtn.addEventListener('click', function () { handleLoadNamedSave(name); });
    
    var delBtn = document.createElement('button');
    delBtn.className = 'icon-btn'; delBtn.type = 'button'; delBtn.textContent = '✕';
...
function renderNotionSaveList() {
  ...
  loadingEl.className = 'named-save-empty'; loadingEl.textContent = '讀取中…';
  ...
  fetch(queryUrl, {
    ...
    if (!result.ok || !result.data.results) {
      var errEl = document.createElement('div'); errEl.className = 'named-save-empty'; errEl.textContent = '讀取失敗：' + JSON.stringify(result.data).slice(0, 200);
      dom.namedSaveList.appendChild(errEl); return;
    }
    notionSavesCache = result.data.results;
    if (notionSavesCache.length === 0) {
      var emptyEl = document.createElement('div'); emptyEl.className = 'named-save-empty'; emptyEl.textContent = 'Notion資料庫中尚無同步記錄';
      dom.namedSaveList.appendChild(emptyEl); return;
    }
    notionSavesCache.forEach(function (page, idx) {
      var props = page.properties || {};
      var titleText = (props['存檔名稱'] && props['存檔名稱'].title && props['存檔名稱'].title[0] && props['存檔名稱'].title[0].plain_text) || '未命名同步記錄';
      var dateText = (props['更新時間'] && props['更新時間'].date && props['更新時間'].date.start) || '';
      var dayNum = (props['遊戲天數'] && props['遊戲天數'].number) || '?';
      var locText = (props['當前地點'] && props['當前地點'].rich_text && props['當前地點'].rich_text.map(function (t) { return t.plain_text; }).join('')) || '';

      var row = document.createElement('div'); row.className = 'named-save-row';
      var info = document.createElement('div'); info.className = 'named-save-info';
      ...
      row.appendChild(info); row.appendChild(loadBtn); dom.namedSaveList.appendChild(row);
    });
  }).catch(function (e) {
    if (currentSaveTab !== 'notion') return;
    dom.namedSaveList.innerHTML = '';
    var errEl = document.createElement('div'); errEl.className = 'named-save-empty'; errEl.textContent = '讀取請求失敗：' + e.message;
...
function handleLoadNotionSave(idx) {
  ...
  if (!page) return;
  var props = page.properties || {};
  var jsonText = props['存檔JSON'] && props['存檔JSON'].rich_text && props['存檔JSON'].rich_text.map(function (t) { return t.plain_text; }).join('');
  if (!jsonText) { alert('這筆記錄沒有可讀取的存檔內容'); return; }
  try {
...
function buildNotionSyncBody() {
  ...
  return {
    ...
    properties: {
      ...
      '人性值': { number: Math.round(gameState.humanity) },
      '飢餓值': { number: Math.round(gameState.hunger) },
      '共鳴值': { number: Math.round(gameState.resonanceValue || 0) },
...
function syncToNotion(silent) {
  if (!CONFIG.NOTION_ENABLED || !CONFIG.NOTION_PROXY_URL || !CONFIG.NOTION_DATABASE_ID) {
    if (!silent) alert('請先填入並儲存 Notion 轉發網址與 Database ID'); return;
  }
  ...
  fetch(CONFIG.NOTION_PROXY_URL, {
    ...
    .then(function (result) {
    if (!silent) {
      if (result.ok) alert('同步成功！請至 Notion 檢查是否新增存檔記錄。');
      else alert('同步失敗，錯誤內容：' + JSON.stringify(result.data).slice(0, 300));
    }
  }).catch(function (e) {
...
function handleNotionSyncNow() { syncToNotion(false); }
function maybeSyncToNotion() { if (gameState.turnCount % NOTION_SYNC_INTERVAL === 0) syncToNotion(true); }

// --- 系統存取與重啟邏輯 ---
function saveStateToLocal() {
  try { localStorage.setItem(STATE_KEY, JSON.stringify(gameState)); } 
  catch (e) {}
}

function restoreState(saved) {
  gameState = Object.assign({}, gameState, saved);
  if (typeof WorldMemory !== 'undefined') gameState.worldMemory = WorldMemory.ensureShape(gameState.worldMemory);
  if (!gameState.charSetup || gameState.charSetup.background !== undefined) {
    gameState.charSetup = {
      name: (saved.charSetup && saved.charSetup.name) || '你',
      ...
      backgroundType: (saved.charSetup && saved.charSetup.backgroundType) || 'combat_survivor',
...
function rebuildNarrativeFromHistory() {
  ...
  for (var i = 0; i < gameState.recentTurns.length; i++) {
    ...
    if (t.action && t.action !== '(開局)') {
      var actionEl = document.createElement('div'); actionEl.className = 'narrative-entry player-action';
      actionEl.textContent = '▸ ' + t.action; dom.narrativeContent.appendChild(actionEl);
    }
    var gmEl = document.createElement('div'); gmEl.className = 'narrative-entry gm-text';
    gmEl.textContent = t.narrative; dom.narrativeContent.appendChild(gmEl);
  }
  if (gameState.isDead) dom.deathScreen.classList.remove('hidden');
...
function handleSaveNotionConfig() {
  ...
  CONFIG.NOTION_PROXY_URL = proxyUrl; CONFIG.NOTION_DATABASE_ID = dbId;
