/* ============================================
   末日黎明：喪屍浩劫 — 物資轉移模組 (transfer.js)
   職責：處理玩家在背包、暫存點、載具及同伴間的物品分配與轉移
   ============================================ */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (cancelBtn) cancelBtn.addEventListener('click', function() { document.getElementById('transfer-modal').classList.add('hidden'); });

  var confirmBtn = document.getElementById('transfer-confirm-btn');
  if (confirmBtn) confirmBtn.addEventListener('click', executeTransferFromModal);
});
...
function openTransferModal(sourceType, sourceId, itemName, maxQty) {
  ...
  if (selectEl.options.length === 0) { alert('沒有其他可轉移的目標！'); return; }
...
function executeTransferFromModal() {
  ...
  if (isNaN(qty) || qty <= 0 || qty > transferState.currentSource.max) { alert('無效的數量'); return; }
  ...
  if (!sourceInv || !targetInv) { alert('找不到來源或目標背包，轉移失敗。'); return; }
